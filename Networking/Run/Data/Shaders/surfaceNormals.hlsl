...
float4 FragmentFunction(v2f_t input) : SV_Target0
{
	...
	float3 surface_normal = NormalColorToVector3(normal_color.xyz); // (0 to 1) space to (-1, -1, 0),(1, 1, 1) space
	surface_normal		  = mul(surface_normal, TBN);

	float4 color = ConvertNormalizedVector3ToColor(surface_normal);
	return color;   
}
